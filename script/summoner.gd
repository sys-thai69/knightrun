# summoner.gd — Mini-boss enemy that spawns minions and teleports away
...
const ACTIVATION_RANGE = 120.0    # Start summoning when player is this close
const MAX_MINIONS = 6             # Maximum total minions at once
const MAX_GHOSTS = 2              # Maximum ghosts at once

var health: int = MAX_HEALTH
var scaled_max_health: int = MAX_HEALTH
var is_dead: bool = false
var is_activated: bool = false    # Only start summoning when player is close
...
func _ready() -> void:
    ...
    # Create floating HP bar instead of top HP bar
    _create_floating_hp_bar()
...
func _create_floating_hp_bar() -> void:
    # Create a small HP bar above the summoner
    _hp_bar = ProgressBar.new()
    _hp_bar.min_value = 0
    _hp_bar.max_value = scaled_max_health
    _hp_bar.value = health
    _hp_bar.show_percentage = false
    _hp_bar.size = Vector2(40, 6)
    _hp_bar.position = Vector2(-20, -35)
    ...
    style_fill.bg_color = Color(0.8, 0.2, 0.8, 0.9)  # Purple for summoner
    ...
    _hp_bar.visible = false  # Hidden until activated
    add_child(_hp_bar)
...
func _physics_process(delta: float) -> void:
    ...
    # Check activation - only summon when player is close
    ...
    # Summon timer — max 6 minions at a time
    summon_timer -= delta
    if summon_timer <= 0 and not _is_summoning and not _is_teleporting and is_activated:
...
func _try_summon() -> void:
    ...
    # Pick a random enemy type, respecting ghost limit
    var scene: PackedScene = _pick_minion_scene()
    if scene == null:
        _is_summoning = false
        return
...
func _pick_minion_scene() -> PackedScene:
    ...
    var ghost_count = 0
    for m in minions:
        if is_instance_valid(m) and m.is_in_group("ghost"):
            ghost_count += 1
    
    # Build list of valid scenes
    var valid_scenes: Array[PackedScene] = []
    for scene in _minion_scenes:
...
func _drop_pushable_block() -> void:
    if _pushable_block_scene:
        ...
        # Use call_deferred to avoid "Can't change state while flushing queries" error
        get_parent().call_deferred("add_child", block)
        block.set_deferred("global_position", global_position + Vector2(0, -10))
