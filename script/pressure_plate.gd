...
var _gate: Node = null
var _activator_count: int = 0  # Track how many activators are on the plate

func _ready() -> void:
    body_entered.connect(_on_body_entered)
    body_exited.connect(_on_body_exited)
    # Use call_deferred so the scene tree is ready
    _setup_gate.call_deferred()
...
func _auto_find_gate() -> void:
    # Auto-detect gate name from plate name (PressurePlate3 → Gate3)
    var target_gate_name = gate_node_name
    var plate_name = name as String
    if plate_name.begins_with("PressurePlate"):
        var suffix = plate_name.substr(13)  # Get everything after "PressurePlate"
        if suffix != "":
            target_gate_name = "Gate" + suffix
        else:
            target_gate_name = "Gate"
    
    # 1) Check parent — maybe plate is a child of gate
    if get_parent().has_method("open_gate"):
        _gate = get_parent()
        return
    # 2) Search siblings for a node named target_gate_name
    if get_parent().has_node(target_gate_name):
        _gate = get_parent().get_node(target_gate_name)
        return
    # 3) Search entire scene for any node named target_gate_name
    var scene_root = get_tree().current_scene
    _gate = _find_node_recursive(scene_root, target_gate_name)
...
func _on_body_exited(body: Node2D) -> void:
    if _is_activator(body) and not toggle_mode:
        _activator_count = max(0, _activator_count - 1)
        # Only deactivate if no activators remain
        if _activator_count <= 0:
            _pressed = false
            _deactivate()
