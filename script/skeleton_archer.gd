...
func _fire_arrow() -> void:
    ...
    arrow.global_position = global_position + Vector2(0, -5)

    # Calculate arc trajectory to hit player
    var target = player_ref.global_position
    var dx = target.x - arrow.global_position.x
    var dy = target.y - arrow.global_position.y
    
    # Time to reach target (estimate based on horizontal distance)
    var time_to_target = abs(dx) / ARROW_SPEED
    if time_to_target < 0.1:
        time_to_target = 0.5  # Minimum flight time
    
    # Calculate initial velocities for parabolic arc
    # vx = dx / t (horizontal velocity)
    # vy = (dy - 0.5 * g * t^2) / t (vertical velocity accounting for gravity)
    var vx = dx / time_to_target
    var vy = (dy - 0.5 * 400.0 * time_to_target * time_to_target) / time_to_target
